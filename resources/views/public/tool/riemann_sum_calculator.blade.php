<form method="POST" action="{{ route('riemann-sum') }}" role="form" enctype="multipart/form-data" class="mt-5">
    @csrf
    <div class="row top-input">
        <div class="form-group col-lg-12 mb-3">
            <div class="d-flex justify-content-between mb-3">
                <label for="function" class="form-label mb-0">{{ __(dynamicString('riemann_sum_input',$language->id)) }}</label>
            ...
            <div class="input-group main-input-group">
                <span class="position-relative p-0 col-12">
                    <input type="text" class="form-control" placeholder="Enter Equation here" id="MathInput" value="x^2" required="required" name="sumof">
        ...
        <div class="form-group col-lg-6 mb-3">
            <label for="variable">{{ __(dynamicString('compute_input',$language->id)) }}</label>
            <select id="variable" class="form-select mt-2" name="compute">
                <option value="right" selected>{{ __(dynamicString('right',$language->id)) }}</option>
                <option value="left">{{ __(dynamicString('left',$language->id)) }}</option>
                <option value="midpoint">{{ __(dynamicString('midpoint',$language->id)) }}</option>
            </select>
        </div>
        <div class="form-group col-lg-6 mb-3">
            <label for="ll" class="form-label">{{ __(dynamicString('compute_with',$language->id)) }}</label>
            <input class="form-control linp"  type="number" name="number" placeholder="Enter number" value="4"/> 
        </div>
