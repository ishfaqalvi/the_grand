<form method="POST" action="{{ route('tripple-integral') }}" role="form" enctype="multipart/form-data" class="mt-5">
    @csrf
    <div class="row top-input">
        ...
        <div class="form-group col-lg-6 mb-3 value-field">
            ...
            <select id="variable" class="form-select mt-2" name="variable">
                <option value="dxdydz">dxdydz</option>
    ...
    <div class="row bonds">
        ...
        <div class="form-group col-lg-3 mb-3 value-field">
            <label for="ul" class="form-label">{{ __(dynamicString('z_upper_limit',$language->id)) }}</label>
            <input class="form-control uinp" type="number" name="z_upperLimit" id="ul3"/> 
        </div>                    
        <div class="form-group col-lg-3 mb-3">
            <label for="ll" class="form-label">{{ __(dynamicString('z_lower_limit',$language->id)) }}</label>
            <input class="form-control linp" type="number" name="z_lowerLimit" id="ll3"/> 
        </div>
    </div>
...
<script>
    function setPreview() {
        $(".equation-preview").html("$$\\int_{" + $("#ll3").val() + "}^{" + $("#ul3").val() + "}\\int_{" + $("#ll2").val() + "}^{" + $("#ul2").val() + "}\\int_{" + $("#ll1").val() + "}^{" + $("#ul1").val() + "}(" + $("#MathInput").val() + ")" + $("#variable").val() + "$$");
    }
    $(function () {
        setPreview(),
        $("#ul1, #ll1,#ul2, #ll2,#ul3, #ll3, #variable").on("change", function () {
            PreviewUpdate();
        }),
        ...
        $("select[name='type']").change(function () {
            ...
            "indefinite" == $(this).val() && ($("#ul1, #ll1,#ul2, #ll2,#ul3, #ll3").val(""), PreviewUpdate());
        });
    });
</script>
